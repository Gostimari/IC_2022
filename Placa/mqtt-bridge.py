...
SENSORS_TOPIC = "zolertia/test"
REGA_TOPIC = "zolertia/rega"

# Address of the Cloud MQTT Broker:
CLOUD_MQTT_URL = "a87pfrh3ne3fk-ats.iot.eu-west-1.amazonaws.com"
CERTIFICATE_AUTH_FILE = "iot_certs/AmazonRootCA1.pem"
CERT_PEM_FILE = "iot_certs/d39fa7c7b144571414ac115b53c1d92ef1901829de7492b6113e0f1064f9fe91-certificate.pem.crt"
PRIVATE_KEY_FILE = "iot_certs/d39fa7c7b144571414ac115b53c1d92ef1901829de7492b6113e0f1064f9fe91-private.pem.key"
...
def on_connect(local_client, userdata, flags, rc):
    ...
    local_client_Rega.subscribe(REGA_TOPIC)
    print("Subscribed to local topic: " + SENSORS_TOPIC + "\n")
    print("Subscribed to local topic (Rega): " + REGA_TOPIC + "\n")
...
def on_cloud_message(cloud_client, userdata, msg):
    ...
    print("Cloud -> Rega: Topic [" + 'zolertia/rega' + "]. Msg \""+str(msg.payload)+"\"")
    local_client_Rega.publish(REGA_TOPIC,str(msg.payload))
...
# Then conect to the Local MQTT Client: (Rega)
local_client_Rega = mqtt.Client()
local_client_Rega.on_connect = on_connect
local_client_Rega.on_message = on_local_message

print("Connecting Rega locally to " + LOCAL_MQTT_URL + "...")
local_client_Rega.connect(LOCAL_MQTT_URL, 1883, 60)
